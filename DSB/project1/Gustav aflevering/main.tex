...
Før jeg kan begynde på opgaverne, starter jeg med at indlæse mine .wav filer med \lstinline{audioread()}. Jeg opretter en struct, der kan indeholde alt information om mine forskellige lydfiler, samt får opdelt s2 op i left og right dele. Dette kan ses på listing \ref{lst:start}. Desuden nulstiller jeg alle tidligere defineret variabler med med \lstinline{clear} og {lstinline{clc}, for ikke at få overlap fra tidligere.

\begin{lstlisting}[caption={Opsætning af MATLAB}, label={lst:start}]
clear;
clc;
...
\begin{lstlisting}{caption={Oprettelse af varaibler til alle structs}, labe=lst:loop}
